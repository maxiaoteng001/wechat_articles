...
from wechatsogou_yf.exceptions import WechatSogouRequestsException, WechatSogouVcodeOcrException, WechatSogouException
from wechatsogou_yf.five import quote
from wechatsogou_yf.identify_image import (
    ws_cache,
...
class WechatSogouAPI(object):
    ...
    def __get_by_unlock(self, url, referer=None, unlock_platform=None, unlock_callback=None,
        ...
        head={'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:47.0) Gecko/20100101 FirePHP/0refox/47.0 FirePHP/0.7.4.1'}
        resp = session.get(url, headers=head)

        if 'antispider' in resp.url or '请输入验证码' in resp.text:
            # print('返回URL为:{}'.format(resp.url))
            if none_if_lock:
                resp = None
            else:
    ...
    def search_gzh(self, keyword, page=1, unlock_callback=None, identify_image_callback=None):
        ...
        对于出现验证码的情况，可以由使用者自己提供：
            ...
            2、也可以 只提供函数 identify_image_callback，这个函数输入验证码二进制数据，输出验证码文字，剩下的由 wechatsogou_yf 包来解决
        注意：
    ...
    def search_article(self, keyword, page=1, timesn=WechatSogouConst.search_article_time.anytime,
        ...
        none_if_lock:如果爬虫被锁定，需要验证时，是否返回None（add by qyt：2018-04-18）
        Returns
        -------
        list[dict]
            {
